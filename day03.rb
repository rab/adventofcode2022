...
input.each_line(chomp: true) do |line|
  ...
  half = line.size / 2
  one = line[0...half].chars
  two = line[half..-1].chars
  overlap = (one & two).first
  value = priorities.index(overlap)
  if testing
    puts "one: #{one.join}"
    puts "two: #{two.join}"
    puts "#{overlap} (#{value})"
...
part2 = 0
input.each_line(chomp: true).each_slice(3) do |a,b,c|
  a = a.chars
  b = b.chars
  c = c.chars
  common = (a & b & c).first
  value = priorities.index(common)
  if testing
    puts a.join,b.join,c.join,"common: #{common} (#{value})"
  end
  part2 += value
end
