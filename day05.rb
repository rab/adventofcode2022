...
input.each_line do |line|
  ...
  if line == "\n"
    mode = :moves
    stacks9001 = stacks.dup
    stacks.keys.each do |key|
      stacks9001[key] = stacks[key].dup
    end
    if debugging
      puts "stacks:",stacks.inspect
      puts "stacks9001:",stacks9001.inspect
    end
    next
  end
  ...
  case mode
  when :crates
    line.chars.each_slice(4).with_index(1) do |crate,stack|
      puts "crate: '#{crate.inspect}'" if debugging
      puts "stack: #{stack}" if debugging
      next if crate.join.chomp.delete(' ').empty?
      if crate.join.match(/\[(.)\]\s/)
        puts "match! #{$1}" if debugging
        stacks[stack].unshift $1
      else
        raise "oops" unless crate.join.delete(' ').to_i == stack
        break
      end
    end
  when :moves
    count, source, dest = line.match(/move (\d+) from (\d+) to (\d+)/).captures.map(&:to_i)
    if debugging
      puts "Before:", "stacks:", stacks.inspect, "stacks9001", stacks9001.inspect
      puts "moving #{count} from #{source} to #{dest}"
    end
    count.times do |move|
      stacks[dest].push stacks[source].pop
    end
    if debugging
      puts "After:", "stacks:", stacks.inspect
    end
    stacks9001[dest].concat stacks9001[source].slice!(-count..-1)
    if debugging
      puts "stacks9001", stacks9001.inspect
    end
  end
...
part1 = stacks.keys.sort.map {|s| stacks[s].last }.join
...
part2 = stacks9001.keys.sort.map {|s| stacks9001[s].last }.join
